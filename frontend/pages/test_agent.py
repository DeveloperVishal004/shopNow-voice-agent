...
function connect() {{
    ...
    ws.onopen = () => {{
        setStatus('Connected — press mic to speak', 'listening');
...
async function startMic() {{
    try {{
        ...
        btn.textContent = '🔴';
        setStatus('Hold mic — speak now, release when done', 'listening');
        log('Mic started');
        visualize();
...
function stopMic() {{
    ...
    btn.textContent = '🎤';
    setStatus('Processing...', 'thinking');
    log('Mic stopped — waiting for Priya');
    ...
    // KEY FIX — tell backend to commit audio and request response
    if (ws && ws.readyState === WebSocket.OPEN) {{
        ws.send(JSON.stringify({{ type: "commit_audio" }}));
        log('Audio sent — waiting for Priya to respond...');
    }}
}}
